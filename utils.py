...
class BitEncoding(Encoding):
    ...
    def encode(data: bytes, image_size: tuple[int, int], output_path: str):
        ...
        if image_size[0] % BitEncoding.BITSIZE[0] != 0 or image_size[1] % BitEncoding.BITSIZE[1] != 0:
            raise ValueError(f'image_size ({image_size}) must be a multiple of {BitEncoding.BITSIZE}')
    ...
    def decode(path: str) -> bytes:
        ...
        im = im.resize((im.size[0] // BitEncoding.BITSIZE[0], im.size[1] // BitEncoding.BITSIZE[1]))
